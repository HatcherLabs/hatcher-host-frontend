        ...
        {usageLoading ? (
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl border p-5" style={{ background: 'var(--bg-elevated)', borderColor: 'var(--border-default)' }}>
              ...
              {(() => {
                const cd = usageData.messages.chart;
                const W = 400, H = 120;
                const PAD = { top: 14, right: 8, bottom: 24, left: 28 };
                const cw = W - PAD.left - PAD.right;
                const ch = H - PAD.top - PAD.bottom;
                const max = Math.max(...cd.map(d => d.count), 1);
                const pts = cd.map((d, i) => ({
                  x: PAD.left + (i / Math.max(cd.length - 1, 1)) * cw,
                ...
                return (
                  <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
                    {yTicks.map(t => (
                      <g key={t.v}>
                        <line x1={PAD.left} y1={t.y} x2={W - PAD.right} y2={t.y} stroke="var(--border-default)" strokeWidth="0.5" strokeDasharray="3 3" />
                        <text x={PAD.left - 4} y={t.y + 3} textAnchor="end" fill="var(--text-muted)" fontSize="8">{t.v}</text>
                      </g>
                    ))}
                    <defs>
                      <linearGradient id="statsUsageGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="var(--color-accent)" stopOpacity="0.25" />
                        <stop offset="100%" stopColor="var(--color-accent)" stopOpacity="0.02" />
                      </linearGradient>
                    </defs>
                    <path d={area} fill="url(#statsUsageGrad)" />
          ...
          <GlassCard>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                {analytics && <TrendBadge pct={msgTrend.pct} direction={msgTrend.direction} />}
              </div>
              <div className="flex items-center gap-1 rounded-lg p-0.5" style={{ background: 'var(--bg-hover)' }}>
                {RANGES.map(r => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${range === r.value ? 'text-white' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
            ...
            {analyticsLoading ? shimmerBars(range === '7d' ? 7 : 30) : analyticsError ? (
                ...
                <div className="h-48">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                      ...
                      <XAxis dataKey="date" tick={{ fill: 'var(--text-muted)', fontSize: 10 }} tickLine={false} axisLine={{ stroke: 'rgba(255,255,255,0.06)' }} interval={range === '30d' ? 4 : 0} />
                      <YAxis tick={{ fill: 'var(--text-muted)', fontSize: 10 }} tickLine={false} axisLine={false} allowDecimals={false} />
                      <RechartsTooltip content={(props: any) => <ChartTooltipContent {...props} theme={theme} />} cursor={{ fill: 'rgba(255,255,255,0.03)' }} />
                      <Bar dataKey="messages" radius={[4, 4, 0, 0]} maxBarSize={40} isAnimationActive={false}>
                        {chartData.map((entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={entry.rawDate === today ? theme.primary : entry.rawDate === analytics.peakDay ? `${theme.primary}bf` : theme.primaryDim}
                          />
                        ))}
                      </Bar>
                    </BarChart>
              ...
              <div className="h-40">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={hourlyData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                    <XAxis dataKey="hour" tick={{ fill: 'var(--text-muted)', fontSize: 9 }} tickLine={false} axisLine={{ stroke: 'rgba(255,255,255,0.06)' }} interval={2} />
                    <YAxis tick={{ fill: 'var(--text-muted)', fontSize: 10 }} tickLine={false} axisLine={false} allowDecimals={false} />
                    <RechartsTooltip content={(props: any) => <ChartTooltipContent {...props} theme={theme} />} cursor={{ fill: 'rgba(255,255,255,0.03)' }} />
                    <Bar dataKey="count" name="Messages" radius={[3, 3, 0, 0]} maxBarSize={20} isAnimationActive={false}>
                      {hourlyData.map((entry, index) => (
                        <Cell key={`hcell-${index}`} fill={entry.isPeak ? theme.primary : theme.primaryDim} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                  ...
                  <div className="h-36">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={responseTimeData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                        <defs>
                          <linearGradient id="statsRtGradient" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor={theme.primary} stopOpacity={0.3} />
                            <stop offset="100%" stopColor={theme.primary} stopOpacity={0} />
                          </linearGradient>
                        </defs>
                        <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                        <XAxis dataKey="date" tick={{ fill: 'var(--text-muted)', fontSize: 10 }} tickLine={false} axisLine={{ stroke: 'rgba(255,255,255,0.06)' }} />
                        <YAxis tick={{ fill: 'var(--text-muted)', fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={(v: number) => formatMs(v)} />
                        <RechartsTooltip
                          content={(props: any) => {
                            ...
                            return (
                              <div className="rounded-lg border border-white/10 px-3 py-2 text-xs shadow-lg" style={{ background: 'var(--bg-elevated)' }}>
                                ...
                                <p style={{ color: theme.primary }}>Avg: {formatMs(props.payload[0].value)}</p>
                              </div>
                            );
                          }}
                        />
                        <Area type="monotone" dataKey="avgMs" stroke={theme.primary} strokeWidth={2} fill="url(#statsRtGradient)" name="Avg Response" isAnimationActive={false} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                )}
