...
export function RecordingDot() {
  return (
    <span className="relative flex h-2.5 w-2.5">
      <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[var(--color-destructive)] opacity-75" />
      <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-[var(--color-destructive)]" />
    </span>
  );
}
...
export function TypingIndicator({ framework }: { framework: string }) {
  const dotColor = ({ openclaw: 'bg-[var(--color-warning)]', hermes: 'bg-[var(--accent)]' } as Record<string, string>)[framework] ?? 'bg-[var(--color-info)]';
  return (
