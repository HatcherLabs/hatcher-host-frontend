...
/**
 * ElizaOSRecentErrorsCard — surfaces ERROR/WARN lines from
 * ElizaOS container stdout logs (docker logs). Parsed server-side,
 * we render up to 10 newest-first with relative timestamps.
 */
export function ElizaOSRecentErrorsCard() {
  ...
  const fetchErrors = useCallback(async () => {
    ...
    try {
      const res = await api.getElizaOSErrors(agent.id);
      if (res.success) {
    ...
    return (
      <GlassCard>
        ...
        <p className="text-xs text-[var(--text-muted)]">
          All clear — no ERROR/WARN entries in the recent container logs.
        </p>
      </GlassCard>
    );
  }
