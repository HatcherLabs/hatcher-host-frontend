...
/**
 * Format an ISO timestamp as "Xm ago" / "Xh ago" / "Xd ago". For
 * errors the relative form is more useful than absolute time because
 * operators usually care whether something just happened vs last week.
 */
...
/**
 * OpenClawRecentErrorsCard — surfaces WARN/ERROR/FATAL lines from
 * the container's most recent `/tmp/openclaw/openclaw-YYYY-MM-DD.log`
 * file. Parsed server-side into {ts, level, message, source}, we
 * render up to 10 newest-first with relative timestamps.
 *
 * When there are zero entries, show a friendly "all clear" state so
 * operators can scan the card and know nothing's burning.
 */
export function OpenClawRecentErrorsCard() {
  ...
  const fetchErrors = useCallback(async () => {
    ...
    try {
      const res = await api.getOpenClawErrors(agent.id);
      if (res.success) {
