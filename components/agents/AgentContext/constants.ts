import type { IntegrationField, IntegrationDef } from './types';
...
export const EXTRA_PLATFORM_INTEGRATIONS: IntegrationDef[] = [
  ...
  // ─── Hermes-specific extra platforms (upstream gateway adapters) ────
  {
    featureKey: 'openclaw.platform.extra',
    stateKey: 'extra.signal',
    name: 'Signal',
    description: 'Connect via signal-cli HTTP bridge (requires signal-cli daemon running on your host).',
    secretPrefix: 'SIGNAL',
    fields: [
      { key: 'SIGNAL_HTTP_URL', label: 'signal-cli URL', type: 'text', placeholder: 'http://127.0.0.1:8080', helper: 'URL of the signal-cli daemon with HTTP mode enabled', required: true },
      { key: 'SIGNAL_ACCOUNT', label: 'Signal Account', type: 'text', placeholder: '+15551234567', helper: 'Phone number registered with signal-cli', required: true },
    ],
  },
  {
    featureKey: 'openclaw.platform.extra',
    stateKey: 'extra.email',
    name: 'Email (IMAP + SMTP)',
    description: 'Agent reads + replies to emails via IMAP/SMTP.',
    secretPrefix: 'EMAIL',
    fields: [
      { key: 'EMAIL_ADDRESS', label: 'Email Address', type: 'text', placeholder: 'agent@example.com', helper: 'The mailbox the agent uses', required: true },
      { key: 'EMAIL_PASSWORD', label: 'Email Password', type: 'password', placeholder: 'App-specific password', helper: 'Use an app password for Gmail/Outlook — NOT your account password', required: true },
      { key: 'EMAIL_IMAP_HOST', label: 'IMAP Host', type: 'text', placeholder: 'imap.gmail.com', required: true },
      { key: 'EMAIL_SMTP_HOST', label: 'SMTP Host', type: 'text', placeholder: 'smtp.gmail.com', required: true },
      { key: 'EMAIL_IMAP_PORT', label: 'IMAP Port', type: 'text', placeholder: '993', helper: 'Default: 993 (TLS)' },
      { key: 'EMAIL_SMTP_PORT', label: 'SMTP Port', type: 'text', placeholder: '587', helper: 'Default: 587 (STARTTLS)' },
      { key: 'EMAIL_POLL_INTERVAL', label: 'Poll Interval (seconds)', type: 'text', placeholder: '15', helper: 'How often the inbox is checked' },
    ],
  },
  {
    featureKey: 'openclaw.platform.extra',
    ...
    secretPrefix: 'TWILIO',
    fields: [
      { key: 'TWILIO_ACCOUNT_SID', label: 'Account SID', type: 'password', placeholder: 'AC...', helper: 'From Twilio Console > Account', required: true },
      { key: 'TWILIO_AUTH_TOKEN', label: 'Auth Token', type: 'password', placeholder: 'Auth token', required: true },
      { key: 'TWILIO_PHONE_NUMBER', label: 'Sender Number (E.164)', type: 'text', placeholder: '+15551234567', helper: 'Twilio number in E.164 format', required: true },
    ],
  },
  {
    featureKey: 'openclaw.platform.extra',
    stateKey: 'extra.dingtalk',
    name: 'DingTalk',
    description: 'Deploy in DingTalk workspaces.',
    secretPrefix: 'DINGTALK',
    fields: [
      { key: 'DINGTALK_CLIENT_ID', label: 'App Key', type: 'password', placeholder: 'App Key', helper: 'From DingTalk Developer Platform', required: true },
      { key: 'DINGTALK_CLIENT_SECRET', label: 'App Secret', type: 'password', placeholder: 'App Secret', required: true },
    ],
  },
  {
    featureKey: 'openclaw.platform.extra',
    stateKey: 'extra.wecom',
    name: 'WeCom (Enterprise WeChat)',
    description: 'Connect to Enterprise WeChat via WebSocket gateway.',
    secretPrefix: 'WECOM',
    fields: [
      { key: 'WECOM_BOT_ID', label: 'Bot ID', type: 'password', placeholder: 'Bot ID', required: true },
      { key: 'WECOM_SECRET', label: 'Bot Secret', type: 'password', placeholder: 'Bot Secret', required: true },
    ],
  },
  {
    featureKey: 'openclaw.platform.extra',
    stateKey: 'extra.weixin',
    name: 'Weixin (Personal WeChat)',
    description: 'Personal WeChat via the iLink bridge.',
    secretPrefix: 'WEIXIN',
    fields: [
      { key: 'WEIXIN_TOKEN', label: 'API Token', type: 'password', placeholder: 'API Token', helper: 'Token from iLink' },
      { key: 'WEIXIN_ACCOUNT_ID', label: 'Account ID', type: 'password', placeholder: 'Account ID', helper: 'At least one of token or account_id is required' },
    ],
  },
  {
    featureKey: 'openclaw.platform.extra',
    stateKey: 'extra.qqbot',
    name: 'QQ Bot',
    description: 'QQ messaging platform (Tencent QQ Open Platform).',
    secretPrefix: 'QQ',
    fields: [
      { key: 'QQ_APP_ID', label: 'App ID', type: 'password', placeholder: 'Bot App ID', required: true },
      { key: 'QQ_CLIENT_SECRET', label: 'Client Secret', type: 'password', placeholder: 'Client Secret', required: true },
    ],
  },
  {
    featureKey: 'openclaw.platform.extra',
    stateKey: 'extra.homeassistant',
    name: 'Home Assistant',
    description: 'Receive state changes + send commands to Home Assistant devices.',
    secretPrefix: 'HASS',
    fields: [
      { key: 'HASS_TOKEN', label: 'Access Token', type: 'password', placeholder: 'Long-Lived Access Token', helper: 'From Home Assistant > Profile > Long-Lived Access Tokens', required: true },
      { key: 'HASS_URL', label: 'Home Assistant URL', type: 'text', placeholder: 'http://homeassistant.local:8123', helper: 'Default: http://homeassistant.local:8123' },
    ],
  },
];
