  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5 mb-3">
        ...
        <span className="text-[11px] text-[#71717a]">Pick a framework</span>
      </div>
  ...
  return (
    <div className="space-y-2.5">
      <div className="flex items-center gap-1.5 mb-3">
        ...
        <span className="text-[11px] text-[#71717a]">Configure</span>
      </div>
      {/* Name field */}
      <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-2">
        <div className="text-[10px] text-[#71717a] mb-1">Agent Name</div>
      ...
      <AnimatePresence>
        {showPersonality && (
          ...
          >
            <div className="text-[10px] text-[#71717a] mb-1">Personality</div>
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.6 }}
              className="text-[11px] text-[#a1a1aa] leading-relaxed"
            >
              {PERSONALITY_TEXT}
            </motion.div>
          </motion.div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5 mb-3">
        ...
        <span className="text-[11px] text-[#71717a]">Chat</span>
      </div>
      {/* User message */}
      <motion.div
      ...
      >
        <div className="bg-white/[0.04] border border-white/[0.08] rounded-xl rounded-bl-md px-3 py-1.5 max-w-[85%]">
          {showDots && (
...
export function OnboardingDemo() {
  ...
  return (
    <motion.div
      ...
      className="relative w-full max-w-md mx-auto mb-6 rounded-2xl border border-white/[0.08] bg-[#0e0e14]/80 backdrop-blur-xl shadow-lg overflow-hidden"
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 pt-4 pb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#71717a]">Deploy an agent in 60 seconds</p>
          </div>
        </div>
        <button
          onClick={dismiss}
          className="p-1 rounded-md hover:bg-white/[0.06] text-[#52525b] hover:text-[#a1a1aa] transition-colors"
        >
        ...
        {LABELS.map((label, i) => (
          ...
          >
            <div className={`h-0.5 rounded-full mb-1 transition-all duration-500 ${
              i === activeScreen ? 'bg-purple-500' : i < activeScreen ? 'bg-purple-500/30' : 'bg-white/[0.06]'
            }`} />
