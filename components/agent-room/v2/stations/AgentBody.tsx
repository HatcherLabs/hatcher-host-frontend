...
import {
  ...
  type GlbAvatarVariant,
} from './avatarModelConfig';
...
export const AVATAR_VARIANTS = [
  ...
  {
    id: 'hatcher-hatchling-service',
    name: 'Hatcher Shell',
    description: 'premium shell avatar',
    accent: '#9ED5E7',
  },
  {
    id: 'hatcher-hatchling-operator',
    name: 'Hatcher Operator',
    description: 'premium operator avatar',
    accent: '#F1C982',
  },
  {
    id: 'hatcher-shell',
    name: 'Hatcher Hatchling',
    description: 'signature shell operator',
    accent: '#9ED5E7',
  },
  {
    id: 'hatcher-core',
    name: 'Signal Hatchling',
    description: 'floating intelligence core',
    accent: '#9ED5E7',
  },
  {
    id: 'openclaw-mech',
    name: 'OpenClaw Operator',
    description: 'tool-heavy shell operator',
    accent: '#F4C95D',
  },
  {
    id: 'openclaw-scout',
    name: 'OpenClaw Scout',
    description: 'fast workflow scout',
    accent: '#9ED5E7',
  },
  {
    id: 'openclaw-heavy',
    name: 'Build Specialist',
    description: 'deploy and ops specialist',
    accent: '#D6B177',
  },
  {
    id: 'openclaw-drone',
    name: 'Orbit Drone',
    description: 'floating monitor avatar',
    accent: '#9ED5E7',
  },
  {
    id: 'hermes-oracle',
    name: 'Hermes Oracle',
    description: 'research and memory core',
    accent: '#AAB7FF',
  },
  {
    id: 'hermes-scribe',
    name: 'Hermes Scribe',
    description: 'memory keeper shell',
    accent: '#9FC1C7',
  },
  {
    id: 'blob',
...
const HATCHER_NATIVE_AVATAR_ORDER: AvatarVariant[] = [
...
const HATCHER_NATIVE_AVATAR_RANK = new Map(
  HATCHER_NATIVE_AVATAR_ORDER.map((variant, index) => [variant, index]),
);
...
export const SELECTABLE_AVATAR_VARIANTS = AVATAR_VARIANTS.filter((variant) =>
  ...
  const aRank = HATCHER_NATIVE_AVATAR_RANK.get(a.id) ?? 100;
  const bRank = HATCHER_NATIVE_AVATAR_RANK.get(b.id) ?? 100;
...
function pickVariant(
  ...
  if (key === 'openclaw')
    return openclaw[seed % openclaw.length] ?? 'hatcher-hatchling-operator';
  if (key === 'hermes') return hermes[seed % hermes.length] ?? 'hatcher-core';
  return 'hatcher-hatchling-service';
}
