...
export function StatsHud({ agent, level, uptimeLabel }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        ◆ {t('statsAgentLabel', { framework: agent.framework.toUpperCase(), level })}
      </div>
      <div className="mt-1.5 text-base font-bold text-gray-100 md:text-xl">
        ...
        <div className="mt-0.5 text-[10px] font-normal text-gray-400 md:text-[11px]">
          {t('uptimeLabel', { uptime: uptimeLabel })}
        </div>
      </div>
      <div className="mt-2.5 grid grid-cols-2 gap-2 text-[11px]">
        <Stat value={agent.messageCount.toLocaleString()} label={t('messagesLabel')} />
        <Stat value={agent.status} label={t('statusLabel')} />
      </div>
    </div>
  );
}
