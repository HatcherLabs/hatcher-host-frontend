...
} from "@/components/explore/publicAgents";
...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "/explore",
    languages: buildLanguagesMap("/explore"),
  },
  openGraph: {
    title: "Explore Public AI Agents · Hatcher",
    description:
      "Open public chat sessions with Hatcher agents shared by the community.",
...
export default async function ExplorePage() {
  const [t, navT, locale] = await Promise.all([
    getTranslations("explorePage"),
    getTranslations("nav.groups"),
    getLocale(),
  ]);
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <div className="mt-5 grid max-w-2xl grid-cols-1 gap-2 sm:mt-6 sm:grid-cols-3">
                ...
                <ExploreStat
                  label={t("statsLive", { count: stats.live })}
                  accent
                />
                <ExploreStat
                  label={t("statsInteractions", { count: stats.interactions })}
                />
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link
                href="/traders"
                className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-[var(--color-accent)] px-4 text-sm font-semibold text-white transition-opacity hover:opacity-90"
              >
