...
function CopyBlock({ text }: { text: string }) {
  ...
  return (
    <div className="relative group rounded-lg bg-[var(--bg-base)] border border-[var(--border-default)] px-4 py-3 pr-12">
      ...
      <button
        onClick={handleCopy}
        className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg text-[var(--text-muted)] hover:text-[var(--color-accent)] hover:bg-[var(--color-accent)]/10 transition-all duration-200 opacity-0 group-hover:opacity-100"
        aria-label="Copy"
      >
...
const GUIDES: Record<string, PlatformGuide> = {
  ...
  whatsapp: {
    ...
    setupTime: '2 min',
    warnings: [
      'Dashboard QR pairing is available for OpenClaw agents.',
      'Hermes pairing opens an interactive terminal bridge in the dashboard because the upstream bridge still runs a TTY wizard.',
    ],
    prerequisites: [
      'A running OpenClaw or Hermes agent',
      'WhatsApp or WhatsApp Business installed on the phone you want to link',
      'Access to WhatsApp > Linked Devices on that phone',
    ],
    steps: [
      {
        title: 'Open the Integrations tab',
        content: 'Go to your agent dashboard, open Integrations, and select WhatsApp.',
      },
      {
        title: 'Start pairing',
        content: 'Click Pair WhatsApp. OpenClaw shows the QR directly; Hermes opens a guided terminal bridge and then shows the QR.',
      },
      {
        title: 'Scan the QR code',
        content: 'On your phone, open WhatsApp > Linked Devices > Link a Device and scan the dashboard QR code.',
      },
      {
        title: 'Set optional allowlist',
        content: 'Add allowed phone numbers if only specific users should be able to message the agent. Leave it empty for an open bot.',
      },
      {
        title: 'Test the agent',
        content: 'Send a WhatsApp message to the linked account and watch the agent logs/status in Hatcher.',
      },
    ],
    hatcherConfig: [
      { field: 'Allowed Phone Numbers', value: '+15551234567', description: 'Optional comma-separated allowlist in E.164 format' },
      { field: 'Pairing Status', value: 'QR / connected', description: 'Managed by Hatcher after you scan the Linked Devices QR code' },
    ],
    testSteps: [
      'Scan the QR code from WhatsApp Linked Devices',
      'Send a WhatsApp message to the linked account',
      'Your agent should respond within a few seconds',
      'Check the Logs tab in Hatcher for message delivery status',
    ],
    faq: [
      { q: 'Do I need Meta Business API credentials?', a: 'No. The current Hatcher dashboard flow uses WhatsApp Linked Devices QR pairing.' },
      { q: 'Can Hermes pair WhatsApp from the dashboard?', a: 'Yes. Hatcher opens the Hermes WhatsApp terminal wizard in a modal and captures the QR for scanning.' },
      { q: 'Can I use my personal WhatsApp number?', a: 'Yes, but a dedicated WhatsApp Business number is usually cleaner for a public bot.' },
      { q: 'Messages are not being received by my agent.', a: 'Confirm the agent is running, the Linked Devices session is still active, and the allowlist includes the sender or is empty.' },
    ],
  },
  slack: {
    name: 'Slack',
      ...
      <div className="mx-auto max-w-3xl">
            ...
            <motion.div key={i} variants={staggerItem}>
              <div className="card glass-noise p-5">
                <div className="flex items-start gap-4">
                  <div className="w-8 h-8 rounded-lg bg-[var(--color-accent)]/15 flex items-center justify-center flex-shrink-0 mt-0.5">
            ...
            <div className="space-y-3">
              {guide.hatcherConfig.map((cfg, i) => (
                <div key={i} className="rounded-xl bg-[var(--bg-card)] border border-[var(--border-default)] p-4">
                  ...
                  <code className="text-sm font-[family-name:var(--font-mono)] text-[var(--color-accent)]">{cfg.value}</code>
            ...
            <ol className="space-y-2">
              {guide.testSteps.map((step, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-[var(--text-secondary)]">
                  <span className="w-5 h-5 rounded-full bg-[var(--color-accent)]/15 flex items-center justify-center flex-shrink-0 mt-0.5 text-[10px] font-bold text-[var(--color-accent)]">
                    {i + 1}
                  </span>
                  {step}
                </li>
        ...
        <motion.div variants={cardVariants}>
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              <ArrowLeft size={18} className="text-[var(--text-muted)] group-hover:text-[var(--color-accent)] transition-colors" />
              <div>
                <p className="text-sm font-semibold text-[var(--text-primary)] group-hover:text-[var(--color-accent)] transition-colors">All Integrations</p>
            ...
            >
              <div className="flex-1">
                <p className="text-sm font-semibold text-[var(--text-primary)] group-hover:text-[var(--color-accent)] transition-colors">API Reference</p>
                <p className="text-xs text-[var(--text-muted)] mt-0.5">Programmatic control of agents</p>
              </div>
              <ArrowRight size={18} className="text-[var(--text-muted)] group-hover:text-[var(--color-accent)] transition-colors" />
            </Link>
          </div>
        </motion.div>
