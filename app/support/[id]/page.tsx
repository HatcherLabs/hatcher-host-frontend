      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="space-y-3">
          {ticket.messages.map((msg, i) => (
            <motion.div
              key={i}
              className={`card glass-noise p-5 ${(msg.role as string) === 'admin' ? 'border-l-2 border-l-[var(--color-accent)]' : ''}`}
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] text-[var(--text-muted)]">{timeAgo(msg.timestamp ?? msg.createdAt)}</span>
              </div>
        ...
        {ticket.status !== 'resolved' && (
          <div className="card glass-noise p-5">
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-xl px-4 py-3 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--color-accent)]/50 focus:ring-1 focus:ring-[var(--color-accent)]/20 transition-colors resize-y mb-3"
            />
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white bg-[var(--color-accent)] hover:bg-[#0891b2] transition-colors disabled:opacity-50"
              >
                {replying ? (
